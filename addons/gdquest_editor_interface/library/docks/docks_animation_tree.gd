...
class AnimationTreeDockDef extends Types.Definition:
	func _init() -> void:
		node_type = "AnimationTreeEditor"

		var custom_script := func(base_node: Node) -> Node:
			var st := EditorInterface.get_base_control().get_tree()
			return Utils.object_get_signal_type(st, "node_removed", "AnimationTreeEditor")
