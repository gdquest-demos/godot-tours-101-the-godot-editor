		...
		var custom_script := func(base_node: Node) -> Node:
			...
			for dock_container in dock_locations:
				for dock in dock_container.get_children():
					...
					if ClassDB.is_parent_class(dock_child.get_class(), "GridMapEditor"):
						return dock
...
class GridMapEditorDef extends Types.Definition:
	func _init() -> void:
		node_type = "GridMapEditor"
		base_reference = Enums.NodePoint.GRID_MAP_DOCK

		resolver_steps = [
			Types.GetChildTypeStep.new("GridMapEditor"),
		]
